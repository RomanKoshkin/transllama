...
class GenericAgent:
    ...
    def push(self, source_segment: Segment, states: Optional[AgentStates] = None) -> None:
        """
    ...
    def pop(self, states: Optional[AgentStates] = None) -> Segment:
        ...
        if is_stateless:
            ...
            action = self.policy()  # NOTE: receive an action (ReadAction/WriteAction) from the agent
        ...
        if action.is_read():
            ...
            if isinstance(action.content, Segment):
                return action.content  # NOTE: if WriteAction, get the content

            segment = SEGMENT_TYPE_DICT[self.target_type](index=0, content=action.content, finished=action.finished)
            states.update_target(segment)  # NOTE: append the new content to the target
            return segment

    def pushpop(self, segment: Segment, states: Optional[AgentStates] = None) -> Segment:
        """
        ...
        """
        # NOTE: we receive an input Segement object from evaluator.__call__
        self.push(segment, states)  # NOTE: update the source, we don't pass states (states=None)

        # NOTE: `self.pop` calls the policy, recieves action, appends new content to the target
        return self.pop(states)  # NOTE: states=None
