...
def get_agent_class(config_dict: Optional[dict] = None) -> GenericAgent:
    ...
    if system_dir is not None:
        print(system_dir, config_name)
        cprint('aaa', color='red')
    ...
    if len(EVALUATION_SYSTEM_LIST) == 0:
        raise RuntimeError("Please use @entrypoint decorator to indicate the system you want to evaluate.")
    if len(EVALUATION_SYSTEM_LIST) > 1:
        if class_name is None:
            raise RuntimeError("--agent-class must be specified if more than one system.")
        # if both --agent-class and --agent:
        for system in EVALUATION_SYSTEM_LIST:
            if f"{system.__module__}.{system.__name__}" == class_name:
                return system
        raise RuntimeError(f"--agent-class {class_name} not found in system list: {EVALUATION_SYSTEM_LIST}")
...
def get_agent_class_from_dir(path: Union[Path, str], config_name: str = "main.yaml") -> GenericAgent:
...
def build_system_args(config_dict: Optional[dict] = None,) -> Tuple[GenericAgent, Namespace]:
    parser = options.general_parser()
    ...
    if check_argument("system_dir"):
        system = build_system_from_dir(check_argument("system_dir"), check_argument("system_config"), config_dict)
    else:
