...
dataset = load_dataset("enimai/MuST-C-ru")
...
N = 4000 # we'll subset 4000 random samples
...
aligned_sentences_valid = alignVerboseSourceToConciseTargetNEW(myaligner, D_valid)

# save a List[Dict[src: trg]], in which src and trg are aligned words
with open(f'../data/mustc2_enru_aligned_dict_{N}_train_.json', 'w') as f:
    f.write(json.dumps(aligned_sentences_train, ensure_ascii=False))
    
with open(f'../data/mustc2_enru_aligned_dict_{N}_valid_.json', 'w') as f:
